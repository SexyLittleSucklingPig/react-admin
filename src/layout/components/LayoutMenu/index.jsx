...
const LayoutMenu = memo(() => {
	...
	const getMenuData = async () => {
		...
		try {
			...
			const menuData = [
				getItem('首页', '/home', <Icons.NodeExpandOutlined />),
				getItem('跳变点设置', '/jumpPointSetting', <Icons.NodeExpandOutlined />),
				getItem('设备管理', '/user', <Icons.MailOutlined />),
			  ]
			// 4. 设置到菜单上
			setMenuList(menuData)
			// 5. 设置到面包屑中
			console.log(findAllBreadcrumb(menuData));
